...
import { findUserByLogin, toSafeUser, touchLastLogin } from "../db/adminDb.js";
...
export function authRoutes(ctx: AppContext): Hono<AuthEnv> {
  ...
  app.post("/login", async (c) => {
    ...
    if (!user.actif) return c.json({ error: "Compte suspendu" }, 403);
  ...
  app.post("/logout", requireAuth(ctx.adminDb, ctx.env.cookieSecure), (c) => {
