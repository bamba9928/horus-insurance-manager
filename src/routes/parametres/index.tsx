  ...
  const handleSubmit = (data: AssureurCreate) => {
    ...
    if (editing) {
      updateMutation.mutate(
        ...
        {
          onSuccess: () => {
            setStatus({ type: "success", msg: "Compagnie modifiée avec succès." });
            closeForm();
          },
          onError: (e) => {
      ...
      createMutation.mutate(payload, {
        onSuccess: () => {
          setStatus({ type: "success", msg: "Compagnie ajoutée avec succès." });
          closeForm();
        },
        onError: (e) => {
  ...
  return (
    <section className="rounded-xl border border-gray-200 bg-white p-5 dark:border-slate-700 dark:bg-slate-800">
                ...
                <div className="flex min-w-0 flex-1 items-center gap-3">
                  {a.api_base_url && (
                    <img
                      src={a.api_base_url}
                      alt={`Logo ${a.nom}`}
                      className="h-10 w-10 shrink-0 rounded-lg border border-gray-200 object-cover dark:border-slate-600"
                      onError={(e) => {
      ...
      <Dialog
        open={isFormOpen}
        onClose={closeForm}
        title={editing ? "Modifier la compagnie" : "Ajouter une compagnie"}
      >
        <AssureurForm
        ...
        onConfirm={async () => {
          if (deleteTarget) {
            await deleteMutation.mutateAsync(deleteTarget.id, {
              onSuccess: () => {
                setStatus({ type: "success", msg: "Compagnie supprimée." });
              },
              onError: (e) => {
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div>
          ...
          <input
            ...
            {...register("portalUrl")}
            className={inputClass}
            placeholder="https://compagnie.sn"
          />
        </div>
        <div>
      ...
      <div>
        <label htmlFor="technicalContact" className={labelClass}>
          Contact ou lien complémentaire
        </label>
        <input
          id="technicalContact"
          type="text"
          ...
          className={inputClass}
          placeholder="Email, téléphone, lien document..."
        />
      </div>
